...
public class Tongxun extends BaseActivity {
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.go_text_btn: //发送文字消息
                if (TextUtils.isEmpty(goEditText.getText().toString())) {
                    Toast.makeText(mActivity, "请先输入信息", Toast.LENGTH_LONG).show();
                } else {
    ...
    public void onMessageEvent(MessageBean messageBean) {
        switch (messageBean.getId()) {
            case 21:                          //接收消息
                ...
                break;
            case BltContant.SEND_TEXT_SUCCESS: //发送消息
                text.append("我:" + goEditText.getText().toString() + "\n");
                goEditText.setText("");
                break;
            case BltContant.SEND_FILE_NOTEXIT:
                Toast.makeText(mActivity, "发送的文件不存在，内存根目录下的test.png", Toast.LENGTH_LONG).show();
                break;
            case BltContant.SEND_FILE_IS_FOLDER:
                Toast.makeText(mActivity, "不能传送一个文件夹", Toast.LENGTH_LONG).show();
                break;
            default:
                break;
        }
